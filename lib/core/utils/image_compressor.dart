  ...
  static Future<File> compress({
    ...
    try {
      ...
      if (image.width > dim || image.height > dim) {
        final scale =
            dim / (image.width > image.height ? image.width : image.height);
      ...
      final outPath = outputPath ??
          '${input.parent.path}/${DateTime.now().millisecondsSinceEpoch}_copy.${input.path.split('.').last}';
      await input.copy(outPath);
      return File(outPath);
    }
  }
}
